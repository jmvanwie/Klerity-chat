...
// ✅ Hook into validateResponse()
case 'legal':
  return validateLegalSummary(responseText);

// ✅ buildSystemPrompt.js (Legal logic added)
export function buildSystemPrompt(taskTypeKey, basePrompt = '') {
  const personas = {
    legal: `You are a legal summarization assistant. You do NOT offer legal advice.
You explain legal topics in structured summaries using accessible language for educational use only.`
    // Add more personas as needed
  };
  return personas[taskTypeKey]
    ? `${personas[taskTypeKey]}

${basePrompt}`
    : basePrompt;
}

// ✅ Unit Test Concept (Jest-style pseudo-test)
const mockLegalResponse = `
**Topic**: Non-Disclosure Agreement (NDA)
**Jurisdiction**: California
**Summary**: An NDA is a legal contract restricting the disclosure of confidential information between parties.
**Disclaimer**: This is not legal advice.

**Topic**: Copyright Basics
**Jurisdiction**: United States (Federal)
**Summary**: Copyright protects original works of authorship once fixed in a tangible medium.
**Disclaimer**: This is not legal advice.`;

const result = validateLegalSummary(mockLegalResponse);
console.assert(result.valid, result.error);

// 🧪 Output: ✅ If valid, silent. ❌ If invalid, logs error